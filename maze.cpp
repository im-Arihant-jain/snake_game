...
void MazeGame::loadMap(const string& mapFileName) {
    ...
    for (int i = 0; i < numRows; i++) {
        gameMap[i][0] = WALLV;
        gameMap[i][numCols - 1] = WALLV;
    }
    for (int i = 0; i < numCols; i++) {
        gameMap[0][i] = WALLH;
        gameMap[numRows - 1][i] = WALLH;
    }
...
bool MazeGame::isValidMove(int newRow, int newCol) {
    return (newRow >= 0 && newRow < numRows && newCol >= 0 && newCol < numCols &&
            gameMap[newRow][newCol] != OBSTACLE && gameMap[newRow][newCol] != WALLH && gameMap[newRow][newCol] != WALLV);
}
...
int main(int argc, char *argv[]) {
    if (argc > 1) {
        ...
        cout<< " Snake game by Arihant Jain "
